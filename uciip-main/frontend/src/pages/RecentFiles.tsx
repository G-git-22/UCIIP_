...
import { getFilesFromBackend } from "@/lib/api"
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      ...
      try {
        ...
        // Map backend file format to frontend FileRecord format
        const recordFiles: FileRecord[] = data.map((f: any) => ({
          id: f.id,
          name: f.name,
          type: f.type.toLowerCase(),
          size: parseInt(f.size) * 1024 * 1024 || 0,
          uploader: { name: f.uploadedBy || 'Unknown', avatar: '' },
          uploadedAt: f.date + 'T12:00:00Z',
          status: 'analyzed' as FileStatus,
          analysisResults: ['Backend Verified'],
          riskLevel: 'medium'
        }))
