...
import { getCasesFromBackend, updateCaseOnBackend } from "@/lib/api"
...
const CaseHistory = () => {
  ...
  useEffect(() => {
    const fetchCases = async () => {
      ...
      try {
        const data = await getCasesFromBackend()
        setCases(data)
      } catch (error) {
  ...
  // Real-time updates simulation (UI only for now)
  useEffect(() => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Case history has been updated from remote database"
      })
    } catch (error) {
      toast({
  ...
  const handleStarToggle = async (caseId: string) => {
    const case_ = cases.find(c => c.id === caseId)
    if (!case_) return

    const newStarred = !case_.starred
    
    // Optimistic update
    setCases(prevCases =>
      prevCases.map(c =>
        c.id === caseId ? { ...c, starred: newStarred } : c
      )
    )

    try {
      await updateCaseOnBackend(caseId, { starred: newStarred })
      toast({
        title: "Case updated",
        description: `Case ${newStarred ? 'starred' : 'unstarred'} on server`
      })
    } catch (error) {
       // Revert on error
       setCases(prevCases =>
  ...
  const handleStatusChange = async (caseId: string, newStatus: CaseRecord['status']) => {
    // Optimistic update
    setCases(prevCases =>
      prevCases.map(case_ =>
        case_.id === caseId 
    ...
    try {
      await updateCaseOnBackend(caseId, { status: newStatus })
      toast({
        title: "Status updated",
        description: `Case status changed to ${newStatus} on server`
      })
    } catch (error) {
