...
import { checkBackendHealth, BackendHealth } from "@/lib/api"
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    checkConn()
    const connInterval = setInterval(checkConn, 15000) // check every 15s
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 bg-card/50 border-cyber-glow/20 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t border-cyber-glow/10">
              <div className="flex items-center justify-between">
                <span className="text-sm font-mono text-muted-foreground">Full-Stack Connectivity</span>
                <div className="flex items-center gap-2">
                  {backendStatus === 'connected' ? (
                    <>
                      <CheckCircle className="h-4 w-4 text-accent" />
                      <span className="text-xs text-accent font-mono">BACKEND ONLINE {backendInfo?.version && `(v${backendInfo.version})`}</span>
                    </>
                    ...
                    <>
                      ...
                      <span className="text-xs text-cyber-danger font-mono">BACKEND DISCONNECTED</span>
                    </>
                  )}
                </div>
              </div>
