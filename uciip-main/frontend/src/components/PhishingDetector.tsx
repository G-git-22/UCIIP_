...
export function PhishingDetector({ isOpen, onClose }: PhishingDetectorProps) {
  ...
  const exportResults = (format: 'pdf' | 'csv' | 'json' = 'json') => {
    ...
    const fileName = `UCIIP-Phishing-Analysis-${timestamp}`
    ...
    if (format === 'json') {
      ...
      const headers = ['URL', 'Risk Level', 'Score', 'Domain', 'Whois Registrar', 'SSL Valid', 'Detected At']
      const rows = filteredUrls.map(u => [
        u.url,
        u.riskLevel.toUpperCase(),
        `${u.riskScore}%`,
        u.decomposition?.domain || 'N/A',
        u.domainIntelligence?.whoisData.registrar || 'N/A',
        u.domainIntelligence?.sslStatus.valid ? 'YES' : 'NO',
        u.timestamp
      ])
      ...
      doc.text('UCIIP - Phishing Analysis Report', 14, 20)
      ...
      doc.setFontSize(14)
      doc.setTextColor(0)
      doc.text('Threat Summary', 14, 45)
      
      const summaryData = [
        ['Total URLs', filteredUrls.length.toString()],
        ['Dangerous', filteredUrls.filter(u => u.riskLevel === 'dangerous').length.toString()],
        ['Suspicious', filteredUrls.filter(u => u.riskLevel === 'suspicious').length.toString()],
        ['Safe', filteredUrls.filter(u => u.riskLevel === 'safe').length.toString()]
      ]
      ...
      doc.text('Detailed URL Analysis', 14, currentY)
      
      const tableData = filteredUrls.map(u => [
        u.url,
        u.riskLevel.toUpperCase(),
        `${u.riskScore}%`,
        u.reasons.join(', ') || 'None'
      ...
      ;(doc as any).autoTable({
        startY: currentY + 5,
        head: [['URL', 'Risk Level', 'Score', 'Detection Reasons']],
        body: tableData,
        theme: 'striped',
        styles: { fontSize: 8 },
        columnStyles: {
