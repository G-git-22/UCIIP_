...
export function EmailChecker({ isOpen, onClose }: EmailCheckerProps) {
  ...
  const exportResults = (format: 'pdf' | 'csv' | 'json') => {
    ...
    const fileName = `UCIIP-Email-Analysis-${timestamp}`

    if (format === 'json') {
      const dataStr = JSON.stringify(emailAnalyses, null, 2)
      ...
      const headers = ['Sender', 'Subject', 'Date', 'Risk Score', 'Risk Level', 'SPF', 'DKIM', 'DMARC']
      const rows = emailAnalyses.map(e => [
        e.sender,
        e.subject,
        e.date,
        `${e.riskScore}%`,
      ...
      doc.setFillColor(10, 15, 25) // Dark theme background for header
      doc.rect(0, 0, 210, 40, 'F')
      
      doc.setFontSize(24)
      doc.setTextColor(0, 255, 255) // Cyan-ish UCIIP color
      doc.text('UCIIP - EMAIL CHECKER', 14, 25)
      
      doc.setFontSize(10)
      doc.setTextColor(150, 150, 150)
      doc.text('OFFICIAL SECURITY ANALYSIS REPORT', 14, 33)
      ...
      const summaryData = [
        ['Total Emails Scanned', emailAnalyses.length.toString()],
        ['Critical Threats Detected', emailAnalyses.filter(e => e.riskLevel === 'threat').length.toString()],
        ['Security Warnings', emailAnalyses.filter(e => e.riskLevel === 'warning').length.toString()],
        ['Safe Documents', emailAnalyses.filter(e => e.riskLevel === 'safe').length.toString()]
      ]
      ...
      autoTable(doc, {
        ...
        body: summaryData,
        theme: 'grid',
        headStyles: { fillColor: [0, 128, 128] }, // Professional teal
      ...
      doc.setFontSize(16)
      doc.text('Detailed Forensics Analysis', 14, currentY)
      
      const tableData = emailAnalyses.map(e => [
        e.sender,
        e.subject,
        e.riskLevel.toUpperCase(),
        `${e.riskScore}%`,
        `SPF: ${e.spfStatus.toUpperCase()}\nDKIM: ${e.dkimStatus.toUpperCase()}\nDMARC: ${e.dmarcStatus.toUpperCase()}`
      ])
      
      autoTable(doc, {
        startY: currentY + 5,
        head: [['Sender', 'Subject', 'Verdict', 'Threat %', 'Auth Results']],
        body: tableData,
        theme: 'striped',
        styles: { fontSize: 8 },
        ...
        didDrawCell: (data) => {
           if (data.section === 'body' && data.column.index === 2) {
             ...
             if (val === 'THREAT') doc.setTextColor(200, 0, 0);
             else if (val === 'WARNING') doc.setTextColor(200, 150, 0);
             else if (val === 'SAFE') doc.setTextColor(0, 150, 0);
           }
        }
      })

      // Detailed Threat Indicators for 'Threat' level emails
      const threats = emailAnalyses.filter(e => e.riskLevel === 'threat')
      if (threats.length > 0) {
        doc.addPage()
        doc.setFontSize(18)
        doc.setTextColor(200, 0, 0)
        doc.text('Critical Threat Indicators', 14, 25)
        doc.setTextColor(0, 0, 0)
        ...
        threats.forEach((threat, i) => {
          ...
          doc.text(`${i + 1}. Source: ${threat.sender}`, 14, pos)
          doc.setFont('helvetica', 'normal')
          doc.setFontSize(10)
          doc.text(`Subject: ${threat.subject}`, 14, pos + 7)
          doc.text(`Threat Percentage: ${threat.riskScore}%`, 14, pos + 14)
          doc.text(`Risk Indicators: ${threat.phishingIndicators.join(', ')}`, 14, pos + 21)
          if (threat.attachments.length > 0) {
             doc.text(`Malicious Attachments: ${threat.attachments.map(a => a.name).join(', ')}`, 14, pos + 28)
          }
          doc.setDrawColor(200, 200, 200)
          doc.line(14, pos + 35, 196, pos + 35)
        })
      }
