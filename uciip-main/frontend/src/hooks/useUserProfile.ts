...
import { getProfileFromBackend, updateProfileOnBackend } from '@/lib/api'
...
export const useUserProfile = () => {
  ...
  useEffect(() => {
    ...
    // Stats and Activity still use localStorage for now as simple persistence
    ...
    if (!savedActivity) {
      const defaultActivity: ActivityLog[] = [
        {
          id: '1',
          action: 'Connected to UCIIP Full-Stack System',
  ...
  const updateProfile = async (field: keyof UserProfile, value: string) => {
    ...
    try {
      await updateProfileOnBackend(newProfile)
      ...
      toast({
        ...
        description: `${field} has been updated on server`,
        duration: 2000,
      })
    } catch (error) {
      toast({
        title: "Update Failed",
        description: "Could not save change to backend",
        variant: "destructive"
      })
    }
  }
  ...
  const saveProfile = async () => {
    setIsLoading(true)
    try {
      await updateProfileOnBackend(profile)
      localStorage.setItem('user-profile', JSON.stringify(profile))
      setLastUpdated(new Date())
      
      addActivity({
        action: 'Saved profile to remote server',
        type: 'setting'
      })
      
      toast({
        title: "Profile Synchronized",
        description: "Your profile has been saved to the backend",
      })
    } catch (error) {
      toast({
        title: "Sync Failed",
        description: "Failed to save profile to backend. Using local storage.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
